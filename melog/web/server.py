...
class WebServer:
    ...
    def publish_tabs(self, tabs) -> None:
        """线程安全：广播新增的分区 tab（前端据此重建左侧切换栏）。"""
        self.view.add_tabs(tabs)
        self.hub.publish({"type": "tabs", "tabs": self.view.tabs})

    def set_tabs(self, tabs) -> None:
        """线程安全：设置分区 tab 集合（历史恢复时整体替换）并广播。"""
        self.view.set_tabs(tabs)
        self.hub.publish({"type": "tabs", "tabs": self.view.tabs})
