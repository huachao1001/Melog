...
def test_log_returns_merged(mlog):
    out = mlog.log({"loss": 0.25})
...
def test_step_auto_increment(mlog):
    mlog.log({"a": 1})
    mlog.log({"a": 2})
    assert mlog.store.snapshot()["a"] == [
        {"step": 0, "value": 1.0},
        {"step": 1, "value": 2.0},
    ]


def test_explicit_step(mlog):
    mlog.log({"a": 1}, step=100)
    assert mlog.store.snapshot()["a"][0]["step"] == 100


# ---------------------------------------------------------------- epoch 支持
def test_log_with_epoch_and_step(mlog):
    """epoch + 当前 epoch 内 step：全局 x 跨 epoch 连续，记录携带 epoch。"""
    for step in range(3):
        mlog.log({"a": step}, epoch=0, step=step)
    for step in range(3):
        mlog.log({"a": 10 + step}, epoch=1, step=step)
    snap = mlog.store.snapshot()["a"]
    assert [p["step"] for p in snap] == [0, 1, 2, 3, 4, 5]
    assert [p["epoch"] for p in snap] == [0, 0, 0, 1, 1, 1]


def test_epoch_only_internal_step_count(mlog):
    """只传 epoch 不传 step：每个 epoch 内部从 0 重新计步。"""
    for _ in range(3):
        mlog.log({"a": 1}, epoch=0)
    for _ in range(2):
        mlog.log({"a": 2}, epoch=1)
    snap = mlog.store.snapshot()["a"]
...
def test_epoch_sticky_after_first_call(mlog):
    """epoch 粘滞：只传一次后，后续不传也记录同一 epoch。"""
    mlog.log({"a": 1}, epoch=7, step=0)
    mlog.log({"a": 2})
    snap = mlog.store.snapshot()["a"]
    assert [(p["step"], p["epoch"]) for p in snap] == [(0, 7), (1, 7)]


def test_explicit_epoch_step_syncs_internal_counter(mlog):
    """显式传 step 后再省略，内部计数从显式值接续。"""
    mlog.log({"a": 1}, epoch=0, step=10)
    mlog.log({"a": 2}, epoch=0)
    snap = mlog.store.snapshot()["a"]
    assert [p["step"] for p in snap] == [10, 11]


def test_epoch_records_persist_with_epoch_key(mlog):
    """启用 epoch 时 JSONL 记录带 epoch 字段。"""
    mlog.log({"a": 1.5}, epoch=2, step=5)
    mlog.finish()
    path = next(mlog.run_dir.parent.glob("**/metrics.melog"))
    rec = json.loads(path.read_text(encoding="utf-8").splitlines()[0])
    assert rec == {"metric": "a", "step": 5, "value": 1.5, "epoch": 2}


def test_no_epoch_records_omit_epoch_key(mlog):
...
def test_log_group_with_epoch(mlog):
    from melog.metrics import Mean, MetricGroup

    group = MetricGroup({"m": Mean()})
    group.update(m=3.0)
    mlog.log_group(group, epoch=1, step=4)
    snap = mlog.store.snapshot()["m"]
    assert snap == [{"step": 4, "value": 3.0, "epoch": 1}]


def test_train_context_returns_progress(mlog):
